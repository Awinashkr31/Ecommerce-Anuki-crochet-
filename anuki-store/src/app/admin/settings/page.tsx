    ...
    <>
      ...
      {loading ? (
        ...
        <div className="space-y-8 max-w-5xl">
          {/* General Settings */}
          <div className="bg-white rounded-3xl shadow-sm border border-neutral-200 p-6 space-y-6">
            <h2 className="text-lg font-black text-neutral-900 border-b border-neutral-100 pb-3">General Settings</h2>
            <div className="space-y-6 max-w-2xl">
              {defaultSettings.filter(s => s.key !== 'free_shipping_threshold' && s.key !== 'cod_enabled').map(s => (
                <div key={s.key}>
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-neutral-200 p-6 space-y-6">
            <h2 className="text-lg font-black text-neutral-900 border-b border-neutral-100 pb-3 flex items-center gap-2">
              <span className="text-rose-600 font-serif">₹</span> Store & Shipping Settings
            </h2>
            
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                <label className="block text-sm font-bold text-neutral-700 mb-2">Minimum Order Value</label>
                <input
                  type="number"
                  value={settings['min_order_value'] || ''}
                  onChange={(e) => updateSetting('min_order_value', e.target.value)}
                  className="w-full border border-neutral-200 rounded-xl p-3 focus:border-rose-500 outline-none"
                />
              </div>
              <div>
                <label className="block text-sm font-bold text-neutral-700 mb-2">Free Delivery Threshold</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-sm font-bold text-neutral-700 mb-2">Delivery Charge</label>
                <input
                  type="number"
                  value={settings['delivery_charge'] || ''}
                  onChange={(e) => updateSetting('delivery_charge', e.target.value)}
                  className="w-full border border-neutral-200 rounded-xl p-3 focus:border-rose-500 outline-none"
                />
              </div>
            </div>
            ...
            <div className="bg-amber-50/60 rounded-2xl border border-amber-200 p-6 mt-6">
              <h3 className="text-sm font-bold text-amber-800 mb-6 flex items-center gap-2">
                <span className="text-amber-700 font-serif">₹</span> Cash on Delivery (COD) Settings
              </h3>
              
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
                <div>
                  <label className="block text-sm font-bold text-neutral-700 mb-2">COD Extra Charge (₹)</label>
                  <input
                    type="number"
                    value={settings['cod_extra_charge'] || ''}
                    onChange={(e) => updateSetting('cod_extra_charge', e.target.value)}
                    className="w-full border border-neutral-200 rounded-xl p-3 focus:border-amber-500 outline-none bg-white"
                  />
                </div>
                <div>
                  <label className="block text-sm font-bold text-neutral-700 mb-2">COD Payment Status</label>
                  <select
                    value={settings['cod_payment_status'] || 'hide'}
                    onChange={(e) => updateSetting('cod_payment_status', e.target.value)}
                    className="w-full border border-neutral-200 rounded-xl p-3 focus:border-amber-500 outline-none bg-white"
                  >
                    <option value="active">Active (Recommended)</option>
                    <option value="hide">Hide Choice (Online Only)</option>
                    <option value="coming_soon">Coming Soon (Display Only)</option>
                  </select>
                </div>
                <div className="text-xs text-amber-700 pb-3 font-medium">
                  Control how Cash on Delivery appears to customers. Set extra charge to 0 for no fee.
                </div>
              </div>
            </div>
          </div>
