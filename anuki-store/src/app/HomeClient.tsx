...
const SectionHeader = ({ title, icon, linkText, linkUrl }: { title: string, icon: string, linkText: string, linkUrl: string }) => (
...
export default function HomeClient({
  ...
  const [recentlyViewed, setRecentlyViewed] = useState<any[]>(randomProducts.slice(0, 4));
    ...
    <div className="min-h-screen bg-white text-neutral-900 font-sans selection:bg-rose-200">
      <main>
        ...
        <section className="px-4 pt-2 pb-4 bg-white">
          <div className="relative rounded-[2rem] overflow-hidden min-h-[420px] md:min-h-[500px] flex flex-col justify-end shadow-sm group">
              ...
              >
                ...
                <div className="absolute inset-0 p-6 flex flex-col justify-end max-w-xl">
                  <div className={`transition-all duration-700 transform ${index === currentHeroSlide ? 'translate-y-0 opacity-100 delay-300' : 'translate-y-4 opacity-0'}`}>
                    ...
                    <h1 className="text-3xl md:text-5xl font-serif text-white mb-3 leading-[1.15] drop-shadow-md">
                      {slide.title}
                    </h1>
                    <p className="text-white/90 text-xs md:text-sm mb-6 font-medium max-w-sm leading-relaxed drop-shadow-sm">
                      {slide.description}
                    </p>
        ...
        <section className="px-4 mt-6 mb-2 max-w-4xl mx-auto">
          <Link href="/products" className="group relative w-full rounded-2xl bg-gradient-to-r from-rose-50 to-[#fff0f3] overflow-hidden shadow-sm hover:shadow-md transition-shadow flex items-center justify-between p-3 px-4 md:px-6 border border-rose-100">
            {/* Sparkles */}
            ...
            <div className="flex items-center gap-3 md:gap-4 z-10">
              ...
              <div className="relative shrink-0 flex items-center">
                <div className="text-2xl md:text-3xl drop-shadow-md group-hover:scale-110 transition-transform animate-[bounce_3s_infinite]">
                  🚚
                </div>
                {/* Speed lines */}
              ...
              <div className="flex flex-col justify-center">
                <div className="flex items-center gap-2">
                  <span className="text-[#e11d48] font-bold text-[8px] md:text-[9px] tracking-wider uppercase bg-white px-2 py-0.5 rounded-full border border-rose-100 hidden sm:block">
                    Special Offer
                  </span>
                  <h3 className="font-serif text-base md:text-lg font-black text-[#1a1a1a] tracking-tight leading-none">
                    Free Shipping
                  </h3>
                </div>
                <p className="text-[#4a4a4a] text-[11px] md:text-xs font-medium leading-tight mt-0.5">
                  on orders over <span className="text-[#e11d48] font-bold text-[13px] md:text-sm">₹{freeDeliveryThreshold}</span>
                </p>
              </div>
            </div>
            ...
            <div className="z-10 shrink-0 ml-2">
              <div className="bg-gradient-to-r from-[#fc4a71] to-[#ff2a5f] text-white font-bold text-[11px] md:text-xs px-4 py-2 rounded-full shadow-sm flex items-center gap-1.5 group-hover:shadow-md transition-shadow">
        ...
        <section className="py-6 px-4 bg-white">
          <div className="flex justify-between items-center mb-5">
            <h2 className="text-lg font-serif font-bold text-neutral-900 flex items-center gap-2">
              <span className="text-[#a43b46]">△</span> Explore Categories
            </h2>
            <Link href="/products" className="text-xs font-bold text-[#a43b46] hover:underline">View all</Link>
          </div>
          <div className="flex gap-4 md:gap-8 overflow-x-auto hide-scrollbar snap-x px-4 -mx-4 max-w-2xl mx-auto">
            {[...categories].sort((a, b) => a.slug === 'flower-pots' ? 1 : b.slug === 'flower-pots' ? -1 : 0).map((cat) => (
              <Link href={`/products?category=${cat.slug}`} key={cat.id} className="snap-start flex flex-col items-center gap-2 min-w-[72px] md:min-w-[96px] group">
                <div className="w-16 h-16 md:w-24 md:h-24 rounded-full overflow-hidden relative border border-[#f0e8e6] shadow-sm group-hover:shadow-md transition-shadow">
                ...
                <span className="text-[11px] md:text-xs font-bold text-neutral-700 text-center group-hover:text-[#a43b46] transition-colors">{cat.name}</span>
              </Link>
            ))}
          </div>
        </section>
        ...
        <section className="py-6 px-4 bg-white border-t border-[#f0e8e6]">
          <div className="flex justify-between items-center mb-5">
            <h2 className="text-lg font-serif font-bold text-neutral-900 flex items-center gap-2">
              <span className="text-[#a43b46]">🕒</span> Recently Viewed
            </h2>
            <Link href="/products" className="text-[10px] font-bold text-neutral-500 uppercase tracking-widest hover:text-[#a43b46] transition-colors">View History</Link>
          </div>
          <div className="flex gap-4 overflow-x-auto hide-scrollbar -mx-4 px-4 pb-4 snap-x">
             {recentlyViewed.map((product) => (
               <div key={product.id} className="snap-start min-w-[150px] w-[150px] md:min-w-[220px] md:w-[220px]">
        ...
        <section className="py-6 px-4 bg-white border-t border-[#f0e8e6]">
          <div className="flex justify-between items-end mb-6">
            <div>
              ...
              <h2 className="text-3xl md:text-4xl font-serif font-bold text-neutral-900 leading-none tracking-tight">Latest Arrivals</h2>
            </div>
        ...
        <section className="py-8 px-4 bg-white">
          <Link href={`https://instagram.com/${instagramHandle.replace('@', '')}`} target="_blank" rel="noopener noreferrer" className="bg-[#2c2829] rounded-[2rem] p-5 flex items-center gap-4 relative overflow-hidden shadow-lg group block">
            ...
            <div className="z-10 text-white flex-1">
              ...
              <h3 className="text-sm md:text-base font-bold mb-1 leading-tight text-[#fcfaf9]">How We Craft Anuki Plushies</h3>
              <p className="text-[10px] md:text-xs text-neutral-400 font-medium">Tap to watch the handmade process</p>
            </div>
        ...
        <section className="py-10 px-4 bg-[#fcf8f7]">
          <div className="max-w-4xl mx-auto bg-white border border-[#f0e8e6] rounded-[2rem] p-6 md:p-10 relative overflow-hidden shadow-sm">
            <p className="text-[10px] font-bold tracking-widest text-[#a43b46] mb-3 uppercase">CO-CREATE WITH US</p>
            <div className="flex flex-col md:flex-row gap-6 mb-6">
              <div className="w-full md:w-3/5">
                <h2 className="text-2xl md:text-4xl font-serif font-bold text-[#3d2b2c] mb-3 leading-[1.15]">Make Your Handmade Gift Yours.</h2>
                <p className="text-[13px] md:text-base text-neutral-600 mb-5 leading-relaxed max-w-sm">
              ...
              <div className="w-full md:w-2/5 relative h-[180px] md:h-auto rounded-2xl overflow-hidden shadow-inner">
            ...
            <div className="grid grid-cols-3 gap-3 mb-6">
              <div className="bg-[#fcf8f7] border border-[#f0e8e6] rounded-xl p-3 flex flex-col items-center justify-center text-center shadow-sm">
                ...
                <span className="text-[10px] md:text-xs font-bold text-[#3d2b2c] leading-tight">Custom Colors</span>
              </div>
              <div className="bg-[#fcf8f7] border border-[#f0e8e6] rounded-xl p-3 flex flex-col items-center justify-center text-center shadow-sm">
                <span className="text-[#a43b46] mb-1.5 text-xl">✨</span>
                <span className="text-[10px] md:text-xs font-bold text-[#3d2b2c] leading-tight">Add Initials</span>
              </div>
              <div className="bg-[#fcf8f7] border border-[#f0e8e6] rounded-xl p-3 flex flex-col items-center justify-center text-center shadow-sm">
                <span className="text-[#a43b46] mb-1.5 text-xl">🧵</span>
                <span className="text-[10px] md:text-xs font-bold text-[#3d2b2c] leading-tight">Handcrafted for You</span>
              </div>
            </div>
            <Link href="/custom" className="w-full md:w-auto md:px-10 bg-[#8c3a44] text-white py-4 rounded-full text-sm font-bold flex items-center justify-center gap-2 hover:bg-[#722e36] transition-colors shadow-lg active:scale-95">
        ...
        <section className="py-10 px-4 bg-white border-t border-[#f0e8e6] max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Link href="/gifts/birthday" className="border border-[#f0e8e6] rounded-[1.25rem] p-3 md:p-4 flex gap-3 items-center hover:bg-rose-50 transition-colors shadow-sm group bg-[#fdfaf9]">
              ...
              <div>
                <div className="font-bold text-[13px] md:text-sm text-[#3d2b2c] mb-0.5 leading-tight">Birthday</div>
                <div className="text-[10px] md:text-[11px] text-neutral-500 font-medium leading-tight">Perfect plushies</div>
              </div>
            </Link>
            <Link href="/gifts/anniversary" className="border border-[#f0e8e6] rounded-[1.25rem] p-3 md:p-4 flex gap-3 items-center hover:bg-rose-50 transition-colors shadow-sm group bg-[#fdfaf9]">
              <div className="w-10 h-10 md:w-12 md:h-12 rounded-full overflow-hidden shrink-0 relative group-hover:scale-110 transition-transform shadow-sm">
                <Image src={getCategoryImage(['flower-pots'], 1) || flowerPotsImg} alt="Anniversary Gifts" fill className="object-cover" />
              </div>
              <div>
                <div className="font-bold text-[13px] md:text-sm text-[#3d2b2c] mb-0.5 leading-tight">Anniversary</div>
                <div className="text-[10px] md:text-[11px] text-neutral-500 font-medium leading-tight">Handmade bouquets</div>
              </div>
            </Link>
            <Link href="/gifts/valentines-day" className="border border-[#f0e8e6] rounded-[1.25rem] p-3 md:p-4 flex gap-3 items-center hover:bg-rose-50 transition-colors shadow-sm group bg-[#fdfaf9]">
              <div className="w-10 h-10 md:w-12 md:h-12 rounded-full overflow-hidden shrink-0 relative group-hover:scale-110 transition-transform shadow-sm">
                <Image src={getCategoryImage(['keychains', 'hair-accessories'], 2) || fallbackImage1} alt="Valentine's Gifts" fill className="object-cover" />
              </div>
              <div>
                <div className="font-bold text-[13px] md:text-sm text-[#3d2b2c] mb-0.5 leading-tight">Valentine's</div>
                <div className="text-[10px] md:text-[11px] text-neutral-500 font-medium leading-tight">Forever roses</div>
              </div>
            </Link>
            <Link href="/gifts/baby-shower" className="border border-[#f0e8e6] rounded-[1.25rem] p-3 md:p-4 flex gap-3 items-center hover:bg-rose-50 transition-colors shadow-sm group bg-[#fdfaf9]">
              <div className="w-10 h-10 md:w-12 md:h-12 rounded-full overflow-hidden shrink-0 relative group-hover:scale-110 transition-transform shadow-sm">
                <Image src={getCategoryImage(['toys'], 3) || fallbackImage2} alt="Baby Shower Gifts" fill className="object-cover" />
              </div>
              <div>
                <div className="font-bold text-[13px] md:text-sm text-[#3d2b2c] mb-0.5 leading-tight">Baby Shower</div>
                <div className="text-[10px] md:text-[11px] text-neutral-500 font-medium leading-tight">Baby-safe toys</div>
              </div>
            </Link>
          </div>
        </section>
        ...
        <section className="py-10 bg-[#fdfaf9] border-y border-[#f0e8e6]">
          ...
          <div className="flex overflow-x-auto hide-scrollbar gap-4 px-4 pb-4 snap-x max-w-7xl mx-auto">
            {[
              ...
              <div key={i} className="snap-center min-w-[280px] md:min-w-[340px] w-[280px] md:w-[340px] bg-white rounded-3xl p-6 border border-[#f0e8e6] shadow-sm flex flex-col justify-between">
                <div>
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={i} className="bg-[#fcf8f7] border border-[#f0e8e6] rounded-2xl overflow-hidden shadow-sm">
                ...
                >
                  <span className="font-bold text-[13px] md:text-sm text-[#3d2b2c]">{faq.q}</span>
        ...
        <section className="px-4 pb-8 bg-white border-t border-[#f0e8e6] pt-8">
          <div className="flex gap-4 overflow-x-auto hide-scrollbar -mx-4 px-4 snap-x max-w-4xl mx-auto">
            <Link href="/products?category=flower-pots" className="snap-start min-w-[280px] md:min-w-[400px] w-[280px] md:w-[400px] h-[180px] md:h-[220px] relative rounded-3xl overflow-hidden group shadow-sm">
               ...
               <div className="absolute bottom-5 left-5 text-white">
                 <p className="text-[9px] md:text-[10px] font-bold tracking-widest uppercase mb-1.5 text-white/90">MOST LOVED</p>
                 <h3 className="text-2xl md:text-3xl font-serif font-bold mb-1.5">Flower Pots</h3>
            ...
            <Link href="/products?category=toys" className="snap-start min-w-[280px] md:min-w-[400px] w-[280px] md:w-[400px] h-[180px] md:h-[220px] relative rounded-3xl overflow-hidden group shadow-sm">
               ...
               <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent"></div>
               <div className="absolute bottom-5 left-5 text-white">
                 <p className="text-[9px] md:text-[10px] font-bold tracking-widest uppercase mb-1.5 text-white/90">CUDDLY FRIENDS</p>
                 <h3 className="text-2xl md:text-3xl font-serif font-bold mb-1.5">Plush Toys</h3>
                 <span className="text-xs font-medium underline underline-offset-4 decoration-white/50 group-hover:decoration-white transition-colors">Shop Online</span>
               </div>
            </Link>
          </div>
