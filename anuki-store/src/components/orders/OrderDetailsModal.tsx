                ...
                <div key={item.id} className={`p-4 flex gap-4 ${i !== 0 ? 'border-t border-neutral-100' : ''}`}>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-3">
                      ...
                      {item.returnRequest && (
                        <div className="text-xs bg-neutral-100 p-2 rounded-lg border border-neutral-200 mt-2">
                          <p className="font-bold text-neutral-900 mb-1">Return Request Info</p>
                          <p>Status: <span className="font-bold text-rose-600 uppercase">{item.returnRequest.status}</span></p>
                          {item.returnRequest.reason && <p className="text-neutral-500 mt-1 italic">"{item.returnRequest.reason}"</p>}
                        </div>
                      )}
                    </div>
                  </div>
            ...
            <div className="bg-white p-5 rounded-2xl border border-neutral-200 shadow-sm">
              ...
              <div className="space-y-3 text-sm text-neutral-600">
                ...
                <div className="mt-4 pt-4 border-t border-neutral-100 bg-neutral-50 -mx-5 -mb-5 p-5 rounded-b-2xl text-xs">
                  ...
                  <p>Status: <span className={`font-bold ${getPaymentStatusText() === 'Paid' ? 'text-emerald-600' : getPaymentStatusText() === 'Payment Pending' ? 'text-amber-600' : 'text-neutral-700'}`}>{getPaymentStatusText()}</span></p>
                  
                  {getPaymentStatusText() === 'Payment Pending' && (
                    <div className="mt-2 p-2 bg-amber-100/50 border border-amber-200 rounded-lg text-amber-800">
                      <p className="font-bold mb-1">Awaiting Payment Confirmation</p>
                      <p>If your payment failed or was interrupted, please try placing the order again or wait a few minutes for the status to update. If money was deducted, it will be automatically refunded by your bank within 5-7 business days.</p>
                    </div>
                  )}

                  {order.payment && order.paymentMethod !== 'COD' && order.paymentMethod !== 'cod' && (
                    <>
                      <p className="mt-2">Gateway: <span className="uppercase">{order.payment.gateway}</span></p>
